    ...
    <div id="section-navigation">
        <ul class="navi">
			...
			if (isset($_GET['cid']) && filter_var($_GET['cid'], FILTER_VALIDATE_INT, ['min_range' => 1])) {
				$cid = $_GET['cid'];
				$pid = NULL;
			} elseif (isset($_GET['pid']) && filter_var($_GET['pid'], FILTER_VALIDATE_INT, ['min_range' => 1])) {
				$pid = $_GET['pid'];
				$cid = NULL;
			} else {
				$pid = NULL;
			...
			while ($cate = mysqli_fetch_array($qu, MYSQLI_ASSOC)) {
				...
				$sql
					= "SELECT cat_id, page_name, page_id FROM `pages` WHERE cat_id ={$cate['cat_id']} ORDER BY position ASC";
